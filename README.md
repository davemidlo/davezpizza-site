...
Minimal static website for [davezpizza.com](https://davezpizza.com/). It uses plain HTML and CSS with no runtime dependencies, build tooling, analytics, or backend.
...
4. Choose `davezpizza-site` and select **Begin setup**.
...
## Connect `davezpizza.com`

`davezpizza.com` is an apex domain. Cloudflare requires an apex domain used with Pages to be an active zone in the same Cloudflare account.

1. In the deployed Pages project, open **Custom domains**.
2. Select **Set up a domain** and enter `davezpizza.com`.
...
5. Once Cloudflare reports the domain active, verify `https://davezpizza.com/` and its certificate.
